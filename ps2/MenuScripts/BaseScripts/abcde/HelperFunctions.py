...
def runscript(file):
    ...
    args = ["perl", "abcde.pl", "-m", "bin2text", "-cm", "abcde::Cartographer", "SLPS_Original/SLPS_254.50", file+"_script.txt", file+"_dump", "-s"]
...
def dumpFile(fileName):
    ...
    #Load the filename
    #and make a string with all the Hex
    fileName = r"G:\TalesHacking\TOD\Files\DAT_ORIGINAL\DAT\MD1\00015.md1"
    f = open(fileName, "rb")
    fText = f.read()
    hexText = binascii.hexlify(fText).decode("utf-8")
